...
namespace Vertimas
{
    public partial class MainForm : Form
    {
        ...
        private void OpenResource(ResourceHolder resource)
        {
			...
			gridEXStrings.Columns["Translated"].Visible=false;
			gridEXStrings.Columns["Error"].Visible=false;
        ...
        private void checkedListBoxLanguages_ItemCheck(object sender, ItemCheckEventArgs e)
        {
			...
			if(gridEXStrings==null)
			{
				// Not populated yet
				return;
			}

			if(gridEXStrings.Columns.Contains(languageHolder.Id))
			{
				gridEXStrings.Columns[languageHolder.Id].Visible=e.NewValue==CheckState.Checked;
			}
        }
        ...
        private void gridEXStrings_DoubleClick(object sender, EventArgs e)
        {
			...
			object value=Convert.ToString(gridEXStrings.Rows[gridEXStrings.SelectedCells[0].RowIndex].Cells[gridEXStrings.SelectedCells[0].ColumnIndex].Value);
			frm.textBoxString.Text=(string)value;
			frm.ShowDialog();

			//gridEXStrings.SetValue(gridEXStrings.Col, frm.textBoxString.Text);
			gridEXStrings.Rows[gridEXStrings.SelectedCells[0].RowIndex].Cells[gridEXStrings.SelectedCells[0].ColumnIndex].Value=frm.textBoxString.Text;
        }
        ...
        private void deleteKeyToolStripMenuItem_Click(object sender, EventArgs e)
        {
			...
			if(MessageBox.Show("Are you sure you want to delete the current key?", "Delete", MessageBoxButtons.YesNoCancel)==
				DialogResult.Yes)
			{
				DataGridViewRow dataRow=gridEXStrings.Rows[gridEXStrings.SelectedCells[0].RowIndex];
				//DataRowView dataRow=gridEXStrings.GetRow().DataRow as DataRowView;
				//dataRow.Row.Delete();
				gridEXStrings.Rows.Remove(dataRow);
			}
        }
        ...
        private void closeToolStripMenuItem_Click(object sender, EventArgs e)
        {
			...
			//gridEXStrings.Rows.re

			gridEXStrings.DataSource=null;
			gridEXStrings.Columns.Clear();
			gridEXStrings.Rows.Clear();
